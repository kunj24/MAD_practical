...
class _ResumeHomePageState extends State<ResumeHomePage> {
  ...
  final projectsController =
      TextEditingController(text: 'Project A — Mobile app; Project B — Backend');
  ...
  void initState() {
    super.initState();
    education = [EducationEntry(institute: 'University', degree: 'B.Tech', year: '2024')];
    experience = [ExperienceEntry(company: 'Acme Corp', role: 'Intern', duration: 'Jun-Aug 2023')];
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: selectedTabIndex == 0 ? buildEditor() : buildPreview(),
          ),
        ],
      ),
    );
  ...
  Widget buildEditor() {
    return ListView(
      ...
      children: [
        ...
        SectionCard(
          title: 'Education',
          action: TextButton.icon(onPressed: addEducation, icon: const Icon(Icons.add), label: const Text('Add')),
          child: Column(children: [for (int i = 0; i < education.length; i++) buildEducationTile(i)]),
        ),
        SectionCard(
          title: 'Experience',
          action: TextButton.icon(onPressed: addExperience, icon: const Icon(Icons.add), label: const Text('Add')),
          child: Column(children: [for (int i = 0; i < experience.length; i++) buildExperienceTile(i)]),
        ),
        SectionCard(
          ...
          children: [
            Row(
              children: [
                Text('Entry ${i + 1}'),
                const Spacer(),
                IconButton(onPressed: () => removeEducation(i), icon: const Icon(Icons.close)),
              ],
            ),
            smallTextField(initial: e.institute, placeholder: 'Institute', onChanged: (v) => setState(() => e.institute = v)),
            smallTextField(initial: e.degree, placeholder: 'Degree', onChanged: (v) => setState(() => e.degree = v)),
            smallTextField(initial: e.year, placeholder: 'Year', onChanged: (v) => setState(() => e.year = v)),
            smallTextField(initial: e.details, placeholder: 'Details', onChanged: (v) => setState(() => e.details = v)),
          ],
        ),
      ),
    );
          ...
          children: [
            Row(
              children: [
                Text('Entry ${i + 1}'),
                const Spacer(),
                IconButton(onPressed: () => removeExperience(i), icon: const Icon(Icons.close)),
              ],
            ),
            smallTextField(initial: ex.company, placeholder: 'Company', onChanged: (v) => setState(() => ex.company = v)),
            smallTextField(initial: ex.role, placeholder: 'Role', onChanged: (v) => setState(() => ex.role = v)),
            smallTextField(initial: ex.duration, placeholder: 'Duration', onChanged: (v) => setState(() => ex.duration = v)),
            smallTextField(initial: ex.details, placeholder: 'Details', onChanged: (v) => setState(() => ex.details = v)),
          ],
        ),
      ),
    );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text('Education:'),
          ...education.map((e) => Text('${e.degree}, ${e.institute} (${e.year})')),
          const SizedBox(height: 10),
          Text('Experience:'),
          ...experience.map((ex) => Text('${ex.role} at ${ex.company} (${ex.duration})')),
          const SizedBox(height: 10),
